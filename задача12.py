# Задача 12. В фермерском хозяйстве в Карелии выращивают чернику. Она растет на круглой грядке,
...
a = [int(input('Введите число ягод на кусте: '))  for i in range(n)]
x = 0
for i in range (1, n-1):
    if (a[i]+a[i+1]+a[i-1] > x):
        x = a[i]+a[i+1]+a[i-1]
print('максимальное колличество ягод -',x)
